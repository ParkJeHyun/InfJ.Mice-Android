...
public class MessageSendMessage_Activity extends Activity implements BaseInterface {
	public static final String TAG = "MessageSendMessage_Activity";	
	// Activity control.
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (StaticData.logined) {
			...
			// get nation key
			new setUserInfo(MessageSendMessage_Activity.this).execute();
		} else {
	...
	public void initView() {
		this.setContentView(R.layout.activity_message_send_message);
	...
	public void modifyXmlResources() {
		...
		case StaticData.KOREA:
			...
			this.total_title.setText(R.string.send_message_ko);
		...
		case StaticData.ENGLISH:
			...
			this.total_title.setText(R.string.send_message_en);
		...
		case StaticData.CHINA:
			...
			this.total_title.setText(R.string.send_message_cn);
	...
	public void setEventListener() {
		...
		this.list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent = new Intent(getApplicationContext(),MessageSendMessageContent_Activity.class);
		...
		this.btn_search_ok.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(edit_search.getText().equals("")){
					...
					new setUserInfoSearch(MessageSendMessage_Activity.this).execute();
				}
			}
		});
			...
			public void onItemSelected(AdapterView<?> parent,
				...
				if(first_t){
					...
					if(nowNation != 0){
						first_n = true;
						nowNation = 0;
						spinner_nation.setSelection(nowNation);						
					}
					new setUserInfoReflash(MessageSendMessage_Activity.this).execute();
				}
			}
...
//						first_t = true;
//						nowMode = 0;
//						spinner_type.setSelection(nowMode);						
//					}
					new setUserInfoSearch(MessageSendMessage_Activity.this).execute();
				}
			}
...
//			this.setIndexValues();
	...
	class getNationKey extends WeakAsyncTask<Void, Void, Void,Activity> {
		...
		protected Void doInBackground(Activity target,Void... params) {
			MCDao dao = new MCDao(MessageSendMessage_Activity.this);
			try {
			...
			if (result) {
				if (MessageSendMessage_Activity.this.nationDtoList != null) {
					MessageSendMessage_Activity.this.nationDtoList.clear();
				}
				MessageSendMessage_Activity.this.nationDtoList = dao.getnationDtoList();
			}
			return null;
		}
		...
		protected void onPostExecute(Activity target,Void param) {
			if(result){
				new getSessionkey(MessageSendMessage_Activity.this).execute();
			}else{
				switch (StaticData.NOWLANGUAGE) {
				case StaticData.KOREA:
				Toast.makeText(getApplicationContext(),
	...
	class getSessionkey extends WeakAsyncTask<Void, Void, Void,Activity> {
		...
		@Override
		protected Void doInBackground(Activity target,Void... params) {
			SearchDAO dao = new SearchDAO(MessageSendMessage_Activity.this);
			try {
				// input id values 1 for test
			...
			if (result) {
				if (MessageSendMessage_Activity.this.sessionDtoList != null) {
					MessageSendMessage_Activity.this.sessionDtoList.clear();
				}
				MessageSendMessage_Activity.this.sessionDtoList = dao.getSessionDtoList();
			}
			return null;
		}
		...
		protected void onPostExecute(Activity target,Void param) {
			if(result){
				setSpinnerAdapter();
				new executeSortList(MessageSendMessage_Activity.this).execute();
			}else{
				switch (StaticData.NOWLANGUAGE) {
				case StaticData.KOREA:
				Toast.makeText(getApplicationContext(),
	...
	class setUserInfo extends WeakAsyncTask<Void, Void, Void,Activity> {
		...
		protected Void doInBackground(Activity target,Void... params) {
			...
			if (result) {
				if (MessageSendMessage_Activity.this.userDtoList != null) {
					MessageSendMessage_Activity.this.userDtoList.clear();
				}
				MessageSendMessage_Activity.this.userDtoList = dao.getSearchUserItemDtoList();
			}
			return null;
		}
		...
		protected void onPostExecute(Activity target,Void param) {
			if(result){
				setUserList();
				new getNationKey(MessageSendMessage_Activity.this).execute();
			}else{
				switch (StaticData.NOWLANGUAGE) {
				case StaticData.KOREA:
				Toast.makeText(getApplicationContext(),
	...
	class setUserInfoReflash extends WeakAsyncTask<Void, Void, Void,Activity> {
		...
		protected Void doInBackground(Activity target,Void... params) {
			...
			if (result) {
				...
				MessageSendMessage_Activity.this.userDtoList.addAll( dao.getSearchUserItemDtoList());
			}
			return null;
		}
