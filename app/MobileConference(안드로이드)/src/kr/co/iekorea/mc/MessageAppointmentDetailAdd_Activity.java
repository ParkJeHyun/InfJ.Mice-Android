...
public class MessageAppointmentDetailAdd_Activity extends Activity implements
		BaseInterface {
	public static final String TAG = "MessageAppointmentDetailAdd_Activity";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (StaticData.logined) {
			...
			// get nation key
			new setUserInfo(MessageAppointmentDetailAdd_Activity.this).execute();
		} else {
	...
	public void setTypeLayout() {
		...
		case corpMode:
			...
			if(this.nowNation != 0){
				this.nowNation = 0;
				this.first_n = true;
				this.spinner_nation.setSelection(nowNation);				
			}
			break;
	...
	public void initView() {
		this.setContentView(R.layout.activity_message_appointment_detail_add);
	...
	public void modifyXmlResources() {
		...
		case StaticData.KOREA:
			...
			this.total_title.setText(R.string.appointments_ko);
		...
		case StaticData.ENGLISH:
			...
			this.total_title.setText(R.string.appointments_en);
		...
		case StaticData.CHINA:
			...
			this.total_title.setText(R.string.appointments_cn);
	...
	public void setEventListener() {
		...
		this.list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				intent.putExtra("user_name", userDtoList.get(position).USER_NAME);
				
				Log.e("Result", "user_cd : "+userDtoList.get(position).USER_CD);
				Log.e("Result", "user_name : "+userDtoList.get(position).USER_NAME);
		...
		this.btn_search_ok.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(edit_search.getText().equals("")){
					...
					new setUserInfoSearch(MessageAppointmentDetailAdd_Activity.this).execute();
				}
			}
		});
...
//					new setUserInfoReflash().execute();
				}
			}
...
//		this.setIndexValues();
	...
	class getNationKey extends WeakAsyncTask<Void, Void, Void,Activity> {
		...
		protected Void doInBackground(Activity target,Void... params) {
			MCDao dao = new MCDao(MessageAppointmentDetailAdd_Activity.this);
			try {
			...
			if (result) {
				if (MessageAppointmentDetailAdd_Activity.this.nationDtoList != null) {
					MessageAppointmentDetailAdd_Activity.this.nationDtoList.clear();
				}
				MessageAppointmentDetailAdd_Activity.this.nationDtoList = dao.getnationDtoList();
			}
			return null;
		}
		...
		protected void onPostExecute(Activity target,Void param) {
			if(result){
				new getSessionkey(MessageAppointmentDetailAdd_Activity.this).execute();
			}else{
				switch (StaticData.NOWLANGUAGE) {
				case StaticData.KOREA:
				Toast.makeText(getApplicationContext(),
	...
	class getSessionkey extends WeakAsyncTask<Void, Void, Void,Activity> {
		...
		@Override
		protected Void doInBackground(Activity target,Void... params) {
			SearchDAO dao = new SearchDAO(MessageAppointmentDetailAdd_Activity.this);
			try {
				// input id values 1 for test
			...
			if (result) {
				if (MessageAppointmentDetailAdd_Activity.this.sessionDtoList != null) {
					MessageAppointmentDetailAdd_Activity.this.sessionDtoList.clear();
				}
				MessageAppointmentDetailAdd_Activity.this.sessionDtoList = dao.getSessionDtoList();
			}
			return null;
		}
		...
		protected void onPostExecute(Activity target,Void param) {
			if(result){
				setSpinnerAdapter();
				new executeSortList(MessageAppointmentDetailAdd_Activity.this).execute();
			}else{
				switch (StaticData.NOWLANGUAGE) {
				case StaticData.KOREA:
				Toast.makeText(getApplicationContext(),
	...
	class setUserInfo extends WeakAsyncTask<Void, Void, Void,Activity> {
		...
		protected Void doInBackground(Activity target,Void... params) {
			...
			if (result) {
				if (MessageAppointmentDetailAdd_Activity.this.userDtoList != null) {
					MessageAppointmentDetailAdd_Activity.this.userDtoList.clear();
				}
				MessageAppointmentDetailAdd_Activity.this.userDtoList = dao.getSearchUserItemDtoList();
			}
			return null;
		}
		...
		protected void onPostExecute(Activity target,Void param) {
			if(result){
				setUserList();
				new getNationKey(MessageAppointmentDetailAdd_Activity.this).execute();
			}else{
				switch (StaticData.NOWLANGUAGE) {
				case StaticData.KOREA:
				Toast.makeText(getApplicationContext(),
	...
	class setUserInfoReflash extends WeakAsyncTask<Void, Void, Void,Activity> {
		...
		protected Void doInBackground(Activity target,Void... params) {
			...
			if (result) {
				...
				MessageAppointmentDetailAdd_Activity.this.userDtoList.addAll( dao.getSearchUserItemDtoList());
			}
			return null;
		}
		...
		protected void onPostExecute(Activity target,Void param) {
			if(result){
				new executeSortList(MessageAppointmentDetailAdd_Activity.this);
			}else{
				switch (StaticData.NOWLANGUAGE) {
				case StaticData.KOREA:
				Toast.makeText(getApplicationContext(),
