...
public class AsyncHttpsTask extends AsyncTask<Void, Void, String> {
     ...
     * @param handler 클래스안에 멤버로 선언해놓은 핸들러변수
     * @param jobj //서버로 전송할 데이터가 들어있는 json 객체
     * @param hnum //Handler Number : 중요. 여기에 지정된 번호대로 다른 종류의 함수를 실행 (1이면 핸들러에서 1번 코드 진행, 2면 핸들러에서 2번 코드 진행)
     * @param Data //순서, 그냥 0 넣어
     */
    ...
    public String Task(String urlString, JSONObject jobj) {
        ...
        try {
            ...
            String encodedJSON = Base64.encodeToString(jobj.toString().getBytes(), 0); //전달하려는 JSON을 Base64로 인코딩해서 보내도록
            ...
            HttpResponse response = httpClient.execute(httpPost); //서버로부터
            String base64ReponseString = EntityUtils.toString(response.getEntity(), HTTP.UTF_8); //서버로부터 받은 String 아직 Base64 상태
            responseString = (Base64.decode(base64ReponseString,0)).toString(); //byte[]로 decode하여 다시 String으로
            Log.i(TAG, (responseString));//디코딩된 String
